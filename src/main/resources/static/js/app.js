 ...
 app.config(function($routeProvider, $locationProvider) {
        ...
        .when("/cryptograph", {
            templateUrl : "views/cryptography.html"
        });
...
//	                alert($rootScope.histories.length);
	...
	var cryptoController =  function($scope,$rootScope,$http){

      $scope.cryptoModel={};  
      console.log("Hello CryptoController");
      
      $scope.encryptText=function()
      {

        var cryptoModel=$scope.cryptoModel;
        if(cryptoModel.originalText==null){
        	$scope.originalTextErr="**Please enter the text first!**";
        	return false;
        }

        if(cryptoModel.encryptKey==null){
        	$scope.encryptKeyErr="**Please enter the key first!**";
        	return false;
        }
        $http({
          url: 'http://127.0.0.1:8080/ats/encrypttext',
          method: "POST",
          data: cryptoModel
        }).then(function(response){
          console.log(response);
          var data=response.data;
          $scope.cryptoModel.encryptedText=data.encryptedText;
        }, 
        function(response){
        	swal("Problem!", "Error Occured !!!"+response, "error");
        });
      }
      ...
      {   
    	var cryptoModel=$scope.cryptoModel;
    	 if(cryptoModel.tobeDecryptedText==null){
        	$scope.tobeDecryptedTextErr="**Please enter the text first!**";
        	return false;
        }

        if(cryptoModel.decryptKey==null){
        	$scope.decryptKeyErr="**Please enter the key first!**";
        	return false;
        }
          $http({
          url: 'http://127.0.0.1:8080/ats/decrypttext',
          method: "POST",
          data: cryptoModel
        }).then(function(response){
        	console.log(response);
            ...
            if((data.decryptedText)=="null"){
            	swal("Sorry!", "You entered wrong text or key", "error");
            }
            else{
            	$scope.cryptoModel.decryptedText=data.decryptedText;
            }
        }, 
        function(response){
          swal("OOps!", "Error Occured!"+response, "warning");
